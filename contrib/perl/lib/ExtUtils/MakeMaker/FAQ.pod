...
In CVS, RCS and SVN you use $Revision: 1.1.1.2 $ (see the documentation of your
version control system for details).  Every time the file is checked
in the $Revision: 1.1.1.2 $ will be updated, updating your $VERSION.

SVN uses a simple integer for $Revision: 1.1.1.2 $ so you can adapt it for your
$VERSION like so:

    ($VERSION) = q$Revision: 1.1.1.2 $ =~ /(\d+)/;
...
and 1.10 to 1.010 which compare properly.
    ...
    $VERSION = sprintf "%d.%03d", q$Revision: 1.1.1.2 $ =~ /(\d+)\.(\d+)/g;

If branches are involved (ie. $Revision: 1.1.1.2 $) its a little more
complicated.

    # must be all on one line or MakeMaker will get confused.
    $VERSION = do { my @r = (q$Revision: 1.1.1.2 $ =~ /\d+/g); sprintf "%d."."%03d" x $#r, @r };

In SVN, $Revision: 1.1.1.2 $ should be the same for every file in the project so
they would all have the same $VERSION.  CVS and RCS have a different
$Revision: 1.1.1.2 $ per file so each file will have a differnt $VERSION.
Distributed version control systems, such as SVK, may have a different
$Revision: 1.1.1.2 $ based on who checks out the file leading to a different $VERSION
