...
sub _succeed {
    return grep { /\S/ } map { s/^\s*//; $_ } split "\n", q[
        ...
        *VERSION = \'1.01';
        ...
        ($VERSION) = sprintf '%i.%03i', split(/\./,('$Revision: 1.1.1.1 $' =~ /Revision: (\S+)\s/)[0]); # $Date: 2009-03-15 19:19:07 $
        ( $VERSION = q($Id: 02_Parse_Version.t,v 1.1.1.1 2009-03-15 19:19:07 ctriv Exp $) ) =~ s/^.*\s+(\d+)\/(\d+)\/(\d+).*$/$1$2$3/; # all one line for MakeMaker
