/* Base configuration file for all MidnightBSD targets.
...
/* Common MidnightBSD configuration. 
   All MidnightBSD architectures should include this file, which will specify
   their commonalities.
...
#define USING_CONFIG_MIDNIGHTBSD_SPEC 1
...
#define MNBSD_SWITCH_TAKES_ARG(CHAR)					\
...
#define MNBSD_WORD_SWITCH_TAKES_ARG(STR)					\
...
#define MNBSD_TARGET_OS_CPP_BUILTINS()					\
  do									\
	...
	MNBSD_NATIVE_TARGET_OS_CPP_BUILTINS();				\
	MNBSD_TARGET_CPU_CPP_BUILTINS();					\
    }									\
  while (0)

#define MNBSD_TARGET_CPU_CPP_BUILTINS() do {} while (0)

#ifdef MIDNIGHTBSD_NATIVE
#define MNBSD_NATIVE_TARGET_OS_CPP_BUILTINS()				\
  do {									\
	builtin_define_with_int_value ("__MidnightBSD_cc_version", MNBSD_CC_VER); \
  } while (0)
#else
#define MNBSD_NATIVE_TARGET_OS_CPP_BUILTINS()				\
  do {} while (0)
...
#ifdef MNBSD_NO_THREADS
#define MNBSD_LIB_SPEC "							\
...
#define MNBSD_DYNAMIC_LINKER "/libexec/ld-elf.so.1"
